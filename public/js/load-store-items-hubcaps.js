...
const loadStoreItems = async (per_page, order) => {
    let htmlDiv = await fetch(urlTo("ratkapne/items")
        + "?page=" + current_page
        + "&order=" + order
        + "&per_page=" + per_page
    )
...
const loadItemData = async (per_page, order) => {
    let data = await fetch(urlTo("api/products/hubcaps/search")
        + "?page=" + current_page
        + "&order=" + order
        + "&per_page=" + per_page
    )
...
const itemDetails = (index) => {
    let mProdId = product_ids[index]
    window.location.href = urlTo("proizvod/" + mProdId + "/ratkapna")
}
