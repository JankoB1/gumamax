...
const loadStoreItems = async (per_page, order) => {
    let htmlDiv = await fetch(urlTo("akumulatori/items")
        + "?page=" + current_page
        + "&order=" + order
        + "&per_page=" + per_page
    )
...
const loadItemData = async (per_page, order) => {
    let data = await fetch(urlTo("api/products/batteries/search")
        + "?page=" + current_page
        + "&order=" + order
        + "&per_page=" + per_page
    )
...
const addToCart = (index,caller) => {
    ...
    fetch(urlTo('/api/products/batteries/' + mProdId)).then((response) => {
        response.json().then(
            (data) => {
                fetch(
...
const itemDetails = (index) => {
    let mProdId = product_ids[index]
    window.location.href = urlTo("proizvod/" + mProdId + "/akumulator")
}
